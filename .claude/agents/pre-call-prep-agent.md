...
27. **Contractor pre-work spreadsheet (Type 3 + Type 5).** For client check-ins and contractor/team calls, check the assigned contractor's weekly meeting pre-work spreadsheet in Google Drive. These contain the most current operational notes: KPIs, budget/spend, status (Good/Bad/Mediocre), issues, and next steps -- often fresher than anything in the database. Resolve the contractor via `reporting_clients.platform_operator`, then search their Drive folder using the Google Drive MCP `search_files` tool. See agent_knowledge SOP: "Contractor Weekly Pre-work Spreadsheet Lookup SOP" for folder IDs and the full lookup chain. Lindsey, Trent, Ahmed, Ade, Scott, and Jordan have sheets. Ahmed and Ade use the full ad management format (Google Ads). Scott's sheet is stale (last updated April 2026). Jordan's sheet tracks task/implementation work, not ad performance.
