...
24. **Mini-app pattern -- split by structure, not line count.** Split an agent into core + `docs/` (or a skill into core + `reference/`) when ANY of these are true, regardless of total size:
    - **Conditional paths:** The agent has multiple workflows where only one runs per invocation (e.g., "if ads build, read ads-requirements; if skill build, read skill-process"). Each path should be a separate docs/ file so only the relevant one is loaded.
    - **Reference data:** The agent contains API templates, query blocks, lookup tables, platform configs, interpretation frameworks, or example libraries. These are reference material -- put them in docs/ so the core prompt stays focused on methodology.
    - **Domain knowledge:** Style guides, audience matrices, pricing models, or other agent-specific knowledge that doesn't change per invocation but isn't needed by other agents. This goes in docs/, not agent_knowledge.

    If the agent is a single linear workflow where every line is relevant every time it runs, a single file is fine even if it's long. The goal is **focus** (only load what's needed), not an arbitrary line limit. See `docs/quality-gates.md` for the directory patterns.
...
26. **Verify GitHub push before declaring build complete.** After writing the agent/skill file, confirm the hook pushed to GitHub. Run `git log --oneline -1` and verify the latest commit includes the new file. If it doesn't, manually `git add` + `git commit` + `git push origin main`. A build is NOT complete until the file is on GitHub. Include the commit hash in your build report.
27. **Domain knowledge belongs in docs/ files, not agent_knowledge.** When building or editing an agent that has reference data (style guides, API templates, platform configs, interpretation frameworks), store that data in the agent's `docs/` directory -- not in `agent_knowledge`. The DB is for corrections, capabilities entries (routing), SOPs shared across agents, and volatile data that changes without a code deploy. Agent-specific reference data lives in GitHub.
28. **Attribution: record who built the agent.** Every agent's `description` field (in both the YAML frontmatter and the `agent_definitions` INSERT) must end with `(Built by [FirstName])` where FirstName is the first name of the person who requested or initiated the build. For admin builds (Peterson, Cade), this can be omitted. For contractor builds, it is MANDATORY. This makes it immediately obvious who created each agent when scanning the agent list.
29. **Update the routing index after every new agent.** After a successful build, append the new agent to `.claude/agent-routing-index.md` in the appropriate category table. Include the agent name and a short "Use when..." trigger description. This file is the quick-reference that Chat/Co-work sessions use for agent discovery. If no existing category fits, create a new one. A build is NOT complete until the routing index is updated.
30. **Contractor builds: Claude in Chrome over MCP.** When building an agent for a contractor (not Peterson or Cade), default to Claude in Chrome browser automation for external service access instead of MCP connections. Contractors don't have authenticated MCP integrations. Exceptions: **Supabase** and **GitHub** MCPs are available to contractors and should be used normally. For everything else (Google Ads, Meta Ads, ClickUp, Gmail, Google Calendar, Square, etc.), use the `chrome-browser-nav` / `chrome-screenshot-pipeline` skill patterns. This rule does NOT apply to agents built for Peterson or Cade.
31. **Universal access by default.** Every agent should work for all users (Peterson, Cade, contractors) unless it is explicitly admin-only (infrastructure, finance, agent-system management). During the build, run the Access Audit from `docs/quality-gates.md` to identify admin-only dependencies. If any exist, add an "Access Requirements" troubleshooting section to the agent's docs explaining what might fail for contractors, why, and how to resolve it (contact the creator, use browser alternative, or skip the step). This applies whether the builder is Peterson, Cade, or a contractor. See `docs/quality-gates.md` "Access Compatibility" section for the full checklist.
32. **Human gate for irreversible external actions.** For any agent action that is irreversible or has wide blast radius -- first-ever outbound message in a new category, destructive platform writes, payment operations -- require explicit human approval REGARDLESS of a clean validator or QC pass. A validator PASS is not a substitute for human sign-off on irreversible actions. Encode this in the agent's workflow as a mandatory pause: present the planned action, state it is irreversible, and require "yes/proceed" before executing.
33. **Existing agents get critic specs on next edit or after a correction/feedback entry.** The retrofit policy: (a) when any Tier 2 or Tier 3 agent's .md is next edited for any reason, a critic spec MUST be added in the same edit (anti-drift rule); (b) when a `type='correction'` or `type='feedback'` entry appears in `agent_knowledge` naming a Tier 2/3 agent by name -- that is the trigger record, add the critic spec on the next session that touches that agent. (c) the one-time retrofit pass for clear-cut Tier 3 CLI agents was completed 2026-08-17 -- see git log for those commits.
34. **Critic spec QC note (learning #4 compliance).** The critic spec generated during Step 4.5 is validated by the normal Step 7 QC pass on the BUILT AGENT FILE -- not by the agent-builder-agent running qc-reviewer on itself. The spec is embedded in the agent's .md, so Step 7's qc-reviewer reads the full agent file including the spec and evaluates it as part of the deliverable. This satisfies learning #4 (builder cannot satisfy its own QC gate) by keeping the QC gate external and on the final artifact.
