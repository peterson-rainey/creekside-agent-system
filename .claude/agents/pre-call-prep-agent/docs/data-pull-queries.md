...
- **Meta:** Use official Meta MCP first: `mcp__claude_ai_Meta_Ads__ads_get_ad_entities` with `level: "campaign"`, `date_preset: "last_30d"`, and metrics fields. Strip `act_` prefix from account ID. If official MCP errors, fall back to AdKit: `mcp__claude_ai_AdKit__adkit_manage` with `entity: "results"` (keep `act_` prefix).
- **Google:** Use `mcp__claude_ai_AdKit__adkit_manage` with `entity: "accounts", platform: "google"` to find the account, then `adkit_manage` with `entity: "results", platform: "google"` for last 30 days

Do NOT leave the performance section blank when MCP tools can provide live data. Flag the source: `[source: Meta Ads MCP, live pull]` or `[source: AdKit MCP, live pull]`
