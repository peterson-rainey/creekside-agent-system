...
2. In `sdr-agent.md`, change `active_partner: [old]` to `active_partner: [slug]`.
3. The `agent-edit-monitor.sh` hook will auto-commit and sync to DB.
4. Verify the change by running: `python3 .claude/agents/sdr-agent/validate_response.py --profile samuel /tmp/test_inactive.txt` (draft mentioning the OLD partner name should BLOCK).
