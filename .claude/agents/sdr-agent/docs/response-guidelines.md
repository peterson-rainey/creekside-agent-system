...
This is about engagement length, not budget qualification -- check the stated starting spend against the Partner Redirect Mode trigger first. If no concrete starting budget has been given, or the stated starting budget is $5K+, this framing is fine: we do 3-month engagements with every client. "We do a 3-month initial engagement so both sides can evaluate fit. If it's working, we keep going. If not, no hard feelings." Don't lower the rate for a trial. If the lead HAS given a concrete starting budget below $5K, route to the active partner per Partner Redirect Mode -- "start small and scale based on performance" language does not override a sub-$5K starting number.
