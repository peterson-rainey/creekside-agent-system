...
- **Non-whitelisted calendar/booking URLs:** Any calendar or booking URL (`calendar.app.google/*`, `calendar.google.com/calendar/*`, `calendly.com/*`) that is not on the approved list is an automatic BLOCK. The only permitted URLs are: https://calendar.app.google/wSdVbfwaJRzkw12E7 (samuel), https://calendly.com/lindsey-bouffard/30min (lindsey), and the active white-label partner's calendar URL for the current profile (from the loaded partner doc). Historical sdr_responses may contain other calendar URLs -- they are context only and must never be copied into a response. On the lindsey profile, Samuel's calendar.app.google URL is ALWAYS a BLOCK; the active partner's calendar (e.g., Keith's calendar.google.com link) IS allowed. The validator enforces this per-profile via a registry in `validate_response.py`; any inactive partner's calendar URL is also a BLOCK.
