...
- **Non-whitelisted calendar/booking URLs:** Any calendar or booking URL (`calendar.app.google/*`, `calendar.google.com/calendar/*`, `calendly.com/*`, `app.reclaim.ai/*`) that is not on the approved list is an automatic BLOCK. The only permitted URLs are: https://calendar.app.google/iwVAR8raqiD9a7dx6 (peterson), https://calendar.app.google/KwQP8WXiFsQgNSdZA (lindsey), Cade's calendar https://calendar.google.com/calendar/appointments/schedules/AcZssZ3j8qaCIjB9v2DojO96hiQDzZOEkUiEnOuBJN1im-dPVtDMjXGehyCUtT_gPaYt0D4i_WxbU037 (peterson profile only, Cade-led calls), and the active white-label partner's calendar URL for the current profile (from the loaded partner doc). Historical sdr_responses may contain other calendar URLs -- they are context only and must never be copied into a response. On the lindsey profile, Peterson's calendar.app.google URL is ALWAYS a BLOCK; the active partner's calendar (Brady's reclaim.ai link) IS allowed. The validator enforces this per-profile via a registry in `validate_response.py`; any inactive partner's calendar URL is also a BLOCK.
- **"Cade" in response text (profile-dependent, ruling 2026-07-23):** Under the **peterson** profile, referencing Cade is ALLOWED using partner framings only ("Cade, my partner," "my co-founder") -- never internal role labels. Cade owns Meta for default-path (higher-value) leads; for partner-routed leads (especially sub-$3K/month ad spend), the active white-label partner is positioned as the Meta specialist instead. Cade's calendar URL IS whitelisted under the peterson profile only (ruling 2026-08-28) -- Cade-led calls book on Cade's calendar (https://calendar.google.com/calendar/appointments/schedules/AcZssZ3j8qaCIjB9v2DojO96hiQDzZOEkUiEnOuBJN1im-dPVtDMjXGehyCUtT_gPaYt0D4i_WxbU037); under lindsey it remains a BLOCK. Under the **lindsey** profile, any "Cade" mention is a BLOCK (solo persona, no agency/co-founder references); lindsey routing targets remain the persona and the active white-label partner only.
...
- **Placeholder brackets:** Any [text in brackets] is a BLOCK. All calendar links now use real URLs, no placeholders needed. Default path calendar is in the loaded profile doc (peterson: https://calendar.app.google/iwVAR8raqiD9a7dx6 | lindsey: https://calendar.app.google/KwQP8WXiFsQgNSdZA). Active partner calendar: from the loaded partner doc.
...
- **Signatures:** Formal sign-offs at end of response. Caught and auto-stripped: bare persona names ("Peterson", "Lindsey"), dash-prefixed names ("- Lindsey", "-- Lindsey"), full names/initials ("Lindsey Bouffard", "Peterson Rainey", "Lindsey B."), closing-plus-name on one line ("Thanks, Lindsey", "Talk soon, Peterson"), multi-line closings ("Best,\nLindsey Bouffard"), and standalone closings ("Best,", "Regards,", "Cheers,"). Applies to both profiles.
...
- Cade's booking calendar URL (peterson profile only, Cade-led calls -- see profile doc)
...
Pass `--profile lindsey` when validating a Lindsey-profile draft. When omitted, the script defaults to peterson behavior.
