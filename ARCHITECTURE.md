...
**Upwork multi-profile (2026-09-19):** The whole `~/upwork-api/` stack is dual-profile: `peterson` (default everywhere, tokens.json, env prefix `UPWORK`, provider `upwork`) and `lindsey` (tokens_lindsey.json, `UPWORK_LINDSEY`, provider `upwork_lindsey`). All scripts take `--profile`: `refresh.py`, `sync_conversations.py`, `upwork_ooo.py` (per-profile message, threshold 3h vs 3.5h, sender keywords, ledger, log, alert name), `send_message.py` (validator profile `lindsey` sends FROM Lindsey's account via `API_PROFILE` map; `samuel` -> peterson). `upwork_conversations.profile` column separates the accounts (973 peterson / 206 lindsey rooms at backfill). Three Lindsey launchd jobs mirror Peterson's: `com.creekside.upwork-lindsey-token-refresh` (12h), `-conversations` (1h), `-ooo` (30min) -- registered in system_registry. The Railway daily `upwork_conversations` job loops both providers and skips a profile gracefully if its auth is down. `upwork_client_stats_sync.py` stays single-account by design (tied to upwork_jobs applications).
...
**Fathom duplicate-recording dedup (rebuilt 2026-09-15):** When multiple people's Fathom notetakers record the same meeting, each ingestion pipeline (webhook for Peterson, `fathom_cade`, `fathom_team`) writes its own `fathom_entries` row -- the unique index is intentionally (url, user_id). The pg_cron job `fathom-dedup-daily` (09:15 UTC) runs `mark_fathom_duplicates()`: for cross-user pairs with overlapping time windows (`meeting_date` + `duration_minutes`) and shared participants, a recording whose overlap is >= 50% of the SHORTER recording's duration is marked `is_canonical=false` / `duplicate_of=<winner>`; the LONGEST recording wins (Peterson only breaks exact-duration ties). Sequential recordings with low overlap (e.g. Cade records the first half, Peterson the second) both stay canonical. The function also self-heals: previously-marked rows that no longer qualify are restored. Non-canonical rows are filtered out of `search_all`, `keyword_search_all`, `get_client_timeline`, `get_topic_360`, and `get_whats_next` (via `COALESCE(is_canonical, true)`), but their raw_content/transcripts remain intact. This replaced the orphaned person-hierarchy version (Peterson-always-wins), which nothing had called since ~April 2026. `fathom_team` still skips any call Peterson or Cade attended at ingestion time (accepted residual person-based rule).
